def generate_base_config(trial_params, fixed_params, suggest_method):
    ...
    for name, values in trial_params.items():
        if isinstance(values, tuple):
            trial_config[name] = suggest_method(name, *values) # e.g., suggest_int('param', 0, 10)
        else:
            trial_config[name] = suggest_method(name, values) # e.g., suggest_categorical('param', [0, 10])
